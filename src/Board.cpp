...
std::string Board::GetFEN() const
{
    ...
    // makruk ไม่ใช้ castling / en passant
...
for(int x=0;x<8;x++)
{
    board[2][x] = new Pawn(Color::Black);
}
...
for(int x=0;x<8;x++)
{
    board[5][x] = new Pawn(Color::White);
}
...
if(piece->GetType() == PieceType::Knight)
{
 ...
 // Penalize unnecessary King movement
if(piece->GetType() == PieceType::King)
{
    positionBonus -= 40;
}
...
auto enemyMoves =
    GenerateCaptureMoves(enemyColor);
...
Piece* Board::MakeTestMove(LegalMove move)
{
    ...
    // update king ชั่วคราว
    if(piece->GetType() == PieceType::King)
    {
        if(piece->GetColor()==Color::White)
            kingPosition[0] = move.to;
        else
            kingPosition[1] = move.to;
    }
...
)
{
    ...
    {
        if(piece->GetColor()==Color::White)
            kingPosition[0] = move.from;
        else
            kingPosition[1] = move.from;
    }
}
